CC=g++
FLAGS=-Wall -Werror -Wextra -std=c++17 -pedantic -g #-fsanitize=address
LIBS=-lstdc++
LIBGTEST=-lgtest -lgtest_main $(LIBS)
OS:=$(shell uname -s)
BIN_FILE=SmartCalc_v2-0
...
clean: uninstall
	@rm -rf *.o ./report ./build ./html ./latex CPPLINT.cfg cpplint.py

build: clean
	@mkdir -p ./build
	$(QMAKE) SmartCalc.pro -o ./build/$(BIN_FILE)_makefile
